...
function help_to_remember()
{
    ...
    if(isset($_POST['email']))
    {
        ...
        if(mysql_num_rows($result)>0)
        {
            ...
            $result=mysql_query("UPDATE users SET password='".md5(SPICE.$password)."' WHERE id='".$arr['id']."'");
...
mail;
        ...
        {
            echo "There is No User Account Associated with the Email You have Submitted. Please, <a href='".BASE."/login/?option=forget'>Try Again</a> with Proper Information.";
        }
    }
    else
    {
...
function login()
{
    ...
    $result=mysql_query("SELECT * FROM users WHERE (username='".$_POST['username']."' OR email='".$_POST['username']."') AND password='".md5(SPICE.$_POST['password'])."'") or die(mysql_error());
    if(mysql_num_rows($result)>0)
    {
        $arr=mysql_fetch_array($result);
        $user_info['id']=$arr['id'];
        ...
        set_flash_message("You have Successfully logged in. <br>TuberPro is Best Viwed in Mozilla Firefiox and Google Chrome Web Browser [<a href='".BASE."/resources'>Info</a>]", 1);
    }
    else
    {
